(() => {
    ...
    (() => {
        ...
        window.addEventListener("click", (function(e) {
            let buttonNext = document.querySelector(".form__button-next");
            let buttonPrev = document.querySelector(".form__button-prev");
            let stepOne = document.querySelector(".main-form__step-one");
            let stepTwo = document.querySelector(".main-form__step-two");
            if (e.target === buttonNext) {
                stepOne.classList.remove("_active");
                stepTwo.classList.add("_active");
            }
            if (e.target === buttonPrev) {
                stepTwo.classList.remove("_active");
            ...
            searchButtons.forEach((button => {
                ...
                } else if (!e.target.closest(".popup") || e.target.closest("button")) {
        ...
        const countryCheckboxes = document.querySelectorAll(".switch input");
        countryCheckboxes.forEach((countryCheckbox => {
            countryCheckbox.addEventListener("change", (e => {
                const checkboxes = e.target.closest(".actions").nextElementSibling.querySelectorAll('input[type="checkbox"]');
                if (e.currentTarget.checked) checkboxes.forEach((checkbox => {
                    if (false == checkbox.checked) checkbox.parentElement.parentElement.classList.add("hide"); else checkbox.parentElement.parentElement.classList.remove("hide");
                })); else checkboxes.forEach((checkbox => {
                    checkbox.parentElement.parentElement.classList.remove("hide");
                }));
            }));
        }));
        let clearAllButtons = document.querySelectorAll(".clear-all");
        clearAllButtons.forEach((clearAllButton => {
            clearAllButton.addEventListener("click", clearAll);
        }));
        function clearAll(e) {
            const checkboxes = e.target.parentElement.nextElementSibling.querySelectorAll('input[type="checkbox"]');
        ...
        function save(e) {
            const checkboxes = e.target.parentElement.parentElement.parentElement.querySelectorAll('.list input[type="checkbox"]');
            const items = e.target.closest(".form__select-option").querySelector(".option-select__items");
            items.innerHTML = "";
            let list = [];
            checkboxes.forEach((checkbox => {
                if (true == checkbox.checked) list.push(checkbox);
            }));
            for (let i = 0; i < list.length; i++) {
                let elem = document.createElement("span");
                elem.innerHTML = list[i].value;
                items.append(elem);
        ...
        searchBoxes.forEach((searchBox => {
            searchBox.addEventListener("keyup", (event => {
                ...
                checkboxes.forEach((checkbox => {
                    if (checkbox.value.toLowerCase().includes(event.currentTarget.value.toLowerCase())) checkbox.parentElement.parentElement.classList.remove("hide"); else checkbox.parentElement.parentElement.classList.add("hide");
                }));
            }));
        }));
        const buildList = (nodes, str) => `<ul>${nodes.map((({name}) => `<li><label title="" for="${name}${str}"><input type="checkbox" value="${name}" name="${str}" id="${name}${str}"><span>${name}</span></label></li>`)).join("")}</ul>`;
        var JSON1 = [ {
            name: "Afghanistan",
            code: "AF"
        }, {
        ...
        var JSON2 = [ {
            ...
            name: "Chilled Organic Pasta"
        }, {
            name: "Gluten Free Foods"
        }, {
            name: "Fruit and Vegetable Powder"
        }, {
        ...
        if (document.querySelector(".page__producer .list")) {
            document.getElementsByClassName("list")[0].innerHTML = buildList(JSON1, "country");
            document.getElementsByClassName("list")[1].innerHTML = buildList(JSON1, "notcountry");
            document.getElementsByClassName("list")[2].innerHTML = buildList(JSON2, "product");
        } else if (document.querySelector(".list")) {
            document.getElementsByClassName("list")[0].innerHTML = buildList(JSON1, "country2");
            document.getElementsByClassName("list")[1].innerHTML = buildList(JSON2, "product2");
        }
        formFieldsInit({
            viewPass: false,
            autoHeight: false
